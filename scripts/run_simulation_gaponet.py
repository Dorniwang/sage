...
"""
Run simulation benchmark with GapONet sim-to-real gap correction.

Same as run_simulation.py but applies GapONet delta correction to
target joint positions before sending them to the simulated robot.
...
Usage:
    python scripts/run_simulation_gaponet.py \
        ...
        --gaponet-model /path/to/gaponet_jit.pt \
        --headless
"""
...
import sys
sys.path.append("/cpfs/workspace/code/sage_lerobot/sage")

from sage.simulation import get_motion_files, get_motion_name, log_message
from sage.simulation_gaponet import JointMotionBenchmarkGapONet


def main():
    parser = argparse.ArgumentParser(description="Benchmark joint motion in Isaac Sim with GapONet correction")
    parser.add_argument("--robot-name", type=str, required=True, help="Robot name (so101)")
    parser.add_argument(
        "--motion-source",
        type=str,
    ...
    # GapONet arguments
    parser.add_argument("--gaponet-model", type=str, default=None,
                        help="Path to GapONet JIT model (.pt). If not provided, runs without correction.")
    parser.add_argument("--gaponet-device", type=str, default="cpu",
                        help="Device for GapONet inference (default: cpu)")
    parser.add_argument("--joint-delta-scale", type=str, default=None,
                        help="Per-joint delta scaling factors, comma-separated. "
                        "E.g. '0.8,1.0,1.0,0.5,0.1,0.1' for SO101")
    ...
    if args.gaponet_model:
        log_message(f"GapONet model: {args.gaponet_model}")
    else:
        log_message("GapONet: DISABLED (no --gaponet-model provided)")

    # Initialize benchmark environment with GapONet
    benchmark = JointMotionBenchmarkGapONet(args)
